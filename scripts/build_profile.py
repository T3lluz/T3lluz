...
def typing_url(p: Profile) -> str:
    ...
    if focus:
        repo = focus[0][0]
        lines.append(_cap(f"Working on {repo.name}" + (f": {repo.description}" if repo.description else "")))
    commit = svg_art.latest_commit(p)
    if commit:
        lines.append(_cap(f"Last commit in {commit['repo']}: {commit['message']}"))
    if len(focus) > 2:
        lines.append(_cap(f"Also working on {focus[1][0].name} and {focus[2][0].name}"))
    if p.top_languages():
        langs = p.top_languages(3)
        lines.append(f"Mostly writing {', '.join(langs[:-1])} and {langs[-1]} lately" if len(langs) > 1
                     else f"Mostly writing {langs[0]} lately")
    lines.append(f"{p.contributions:,} contributions in the last year")
...
def language_row(p: Profile) -> str:
    ...
    imgs = "\n".join(
        f'<img src="{icons.LANGUAGES[n][1]}" alt="{n}" title="{n}: {pct:.0f}%" '
...
def readme(p: Profile, title_widths: dict[str, int]) -> str:
    ...
    about_badges = []
    if focus:
        about_badges.append(badge("Working on", focus[0].name, "1F6FEB",
                                  icons.language_slug(focus[0].language), focus[0].url))
    for name, pct, _ in recent_langs[:3]:
        if name != "Other":
            about_badges.append(badge(name, f"{pct:.0f}%", "388BFD", icons.language_slug(name)))
    commit = svg_art.latest_commit(p)
    if commit:
        about_badges.append(badge("Last commit", f"{commit['repo']}, {svg_art.age(commit['date'], p.generated_at)}",
                                  "1F6FEB", "git", commit["url"]))
    about_badges.append(badge("Repos active this month", str(len(p.active)), "388BFD", "github",
    ...
    ai = ["lmstudio", "hermes"] + [k for k in p.tech if k in AI_DETECTED]
...
<p>
  Bachelor in computer engineering, from {p.location or "Norway"}.<br />
  Working on {projects}.
</p>

<img width="910" src="{typing_url(p)}" alt="What I'm working on" />
...
<div align="center">
  {indent(title("pulse", "Activity"), 2)}<br /><br />
...
<div align="center">
  {indent(title("stack", "Stack"), 2)}<br /><br />
  <table align="center" width="100%">
    <tbody>
      <tr valign="top">
